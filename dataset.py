...
class VOCDataset(Dataset):
    # This is used only in the training procedure: core function is encode and __getitem__.
    # TODO Figure out the data preprocessing.
    ...
    def __getitem__(self, idx):
        ...
        # For debug, check the ground data are well-cropped by the ground truth bounding box.
        ...
        # TODO check the encode function.
    ...
    def encode(self, boxes, labels):
        # This function will be used at every data ireations, and it aims to normalize the bounding box to 0-1 scale.
        ...
        # Target: transform the ground truth 4D vector into the format (x_offset[0-1], y_offset[0-1], normalized_height, normlized_width)
        ...
        for b in range(boxes.size(0)):
            ...
            # TODO 
            # We follow darknet's implementation, so the column index will be added on the x-axis, on the other hand, row index is andded on the y-axis.
            for k in range(B):
                s = 5 * k
                target[j, i, s  :s+2] = xy_normalized
                target[j, i, s+2:s+4] = wh
                target[j, i, s+4    ] = 1.0
                #target[i, j, s  :s+2] = xy_normalized
                #target[i, j, s+2:s+4] = wh
                #target[i, j, s+4    ] = 1.0
    ...
    # Jitter is set to 0.3, but in pytorhc implementation is 0.6.
...
def create_dataloader(args, image_dir, train_label, val_label):
    dataloaders = {}
    dataset_size = {} 
    train_dataset = VOCDataset(True, image_dir, train_label)
    dataloaders['train'] = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.workers, pin_memory=True)
    val_dataset = VOCDataset(False, image_dir, val_label)
    dataloaders['test'] = torch.utils.data.DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.workers, pin_memory=True)
    dataset_size['train'] = len(train_dataset)
    dataset_size['test'] = len(val_dataset)
    table_data = [('train', str(len(train_dataset))),('test', str(len(val_dataset)))]
    data_table = ['Type', 'Size']
    # should be wrapped by dataloaders:::::
