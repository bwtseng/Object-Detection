...
class RetinaNet:
	...
	def _build_detection_graph(self):
		...
		with tf.variable_scope('inference'):
			...
			if self.mode == 'train':
				...
				### Exclude the object/non-object class.
	...
	def _get_abbox(self, size, pshape):
		## [32, 64, 128, 256, 512]
		## self.anchors[0], p3shape (feature map shape)
		if self.data_format == 'channels_last':
		...
		### Each grid cell gives its center number.
	...
	def _compute_one_image_loss(self, pbbox_yx, pbbox_hw, abbox_y1x1, abbox_y2x2,
		...
		### copy for the same amounts as anchor box.
