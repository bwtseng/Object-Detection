...
import numpy as np
# ******************************************
# VOC class names and BGR color for plotting
# ******************************************
VOC_CLASS_BGR = {
    'aeroplane': (128, 0, 0),
    'bicycle': (0, 128, 0),
...
class YOLODetector:
    def __init__(self, net, class_name_list=None, mean_rgb=[122.67891434, 116.66876762, 104.00698793],
        conf_thresh=0.1, prob_thresh=0.1, nms_thresh=0.5, gpu_id=0):
        ...
        self.yolo = Darknet('yolov1-tiny.cfg', img_size=448)
        self.yolo.load_darknet_weights("tiny-yolov1.weights")
        #self.yolo = Darknet('yolov1.cfg', img_size=448)
        #self.yolo.load_darknet_weights("yolov1.weights")
        self.yolo.cuda()
    ...
    def detect(self, image_bgr, image_size=448):
        ...
        for b in range(boxes_normalized.size(0)):
            ...
            #x1, x2 = 448 * box_normalized[0], 448 * box_normalized[2] # unnormalize x with image width.
            #y1, y2 = 448 * box_normalized[1], 448 * box_normalized[3] # unnormalize y with image height.  
    ...
    def decode(self, pred_tensor):
        ...
        for i in range(S): # for x-dimension.
            for j in range(S): # for y-dimension.
                ...
                class_score, class_label = torch.max(pred_tensor[j, i, :20], 0)
                for b in range(B):
                    #conf = pred_tensor[j, i, 5*b + 4]
                    conf = pred_tensor[j, i, 20+b]
                    ...
                    box = pred_tensor[j, i, 22+(4*b) : 22+(4*b) + 4]

                    #box = pred_tensor[i, j, 5*b : 5*b + 4]
                    #box = pred_tensor[i, j, 20+(5*b) : 20+(5*b) + 4]
        ...
        for i in range(S): # for x-dimension.
            for j in range(S): # for y-dimension.
                class_score, class_label = torch.max(pred_tensor[i, j, 5*B:], 0)
                #class_score, class_label = torch.max(pred_tensor[j, i, :20], 0)
                for b in range(B):
                    #conf = pred_tensor[i, i, 5*b + 4]
                    #conf = pred_tensor[j, i, 20+b]
                    conf = pred_tensor[i, j, 5*b + 4]
                    #if conf < self.conf_thresh:
                    #    continue
                    prob = conf * class_score
                    if float(prob) < self.prob_thresh:
                    ...
                    #box = pred_tensor[i, j, 5*b : 5*b + 4]
                    #box = pred_tensor[i, j, 20+(5*b) : 20+(5*b) + 4]
                    # Why it's j, i should be checked, I found it with the observation from the results using (i, ).
                    # The location of the bounding box is put on the other side of the image.  Thus, I directly change it location.
                    x0y0_normalized = torch.FloatTensor([j, i]) * cell_size # cell left-top corner. Normalized from 0.0 to 1.0 w.r.t. image width/height.
                    xy_normalized = box[:2] * cell_size + x0y0_normalized   # box center. Normalized from 0.0 to 1.0 w.r.t. image width/height.
                    #xy_normalized = (box[:2] * cell_size + x0y0_normalized)   # box center. Normalized from 0.0 to 1.0 w.r.t. image width/height.
                    #xy_normalized[1] *= h 
                    #xy_normalized[0] *= w
                    #wh_normalized = box[2:]
                    wh_normalized = torch.pow(box[2:], 2) # Box width and height. Normalized from 0.0 to 1.0 w.r.t. image width/height.
                    #wh_normalized[0] *= w
                    #wh_normalized[1] *= h
                    box_xyxy = torch.FloatTensor(4) # [4,]
                    box_xyxy[:2] = xy_normalized - 0.5 * wh_normalized  # left-top corner (x1, y1).
